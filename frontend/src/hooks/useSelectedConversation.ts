...
function mapUserToConversation({
  ...
  const mappedMessages: MappedMessage[] = messages.map((message) => {
    // 백엔드 Message 모델에 image, video 속성이 추가로 있다고 가정하고 타입을 단언합니다.
    // (만약 store/useChatStore.ts의 Message 타입에 추가하셨다면 as any는 제거하셔도 됩니다)
    ...
    return {
      ...
      imageUrl: msg.image || msg.media, // 백엔드 필드명(image 또는 media)에 맞게 매핑합니다.
      videoUrl: msg.video,
    };
  });
